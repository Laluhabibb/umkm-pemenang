...
<body id="page-top">
    ...
    <tbody>
        ...
        <tr>
            <td><?php echo $no; ?></td>
            <td><b><a href="../maps.php?id_umkm=<?php echo $d['id_umkm']; ?>" target="_blank"><?php echo htmlspecialchars($d['nama_umkm']); ?></a></b></td>
            <td><?php echo htmlspecialchars($d['nama_pemilik']); ?></td>
            <td><?php echo htmlspecialchars($d['nama_kategori']); ?></td>
            <td><?php echo htmlspecialchars(strlen($d['alamat']) > 30 ? substr($d['alamat'], 0, 30) . '...' : $d['alamat']); ?></td>
            <td><?php echo htmlspecialchars(strlen($d['deskripsi']) > 50 ? substr($d['deskripsi'], 0, 50) . '...' : $d['deskripsi']); ?></td>
            <td><?php echo htmlspecialchars(strlen($d['kontak']) > 15 ? substr($d['kontak'], 0, 15) . '...' : $d['kontak']); ?></td>
            <td><?php echo !empty($d['no_ktp']) ? htmlspecialchars($d['no_ktp']) : '-'; ?></td>
            <td><?php echo !empty($d['nib']) ? htmlspecialchars($d['nib']) : '-'; ?></td>
            <td><?php echo !empty($d['npwp']) ? htmlspecialchars($d['npwp']) : '-'; ?></td>
            <td><?php echo !empty($d['tanggal_berdiri']) ? htmlspecialchars($d['tanggal_berdiri']) : '-'; ?></td>
            <td>
                <?php if (!empty($d['foto'])): ?>
                    <img src="../images/<?php echo htmlspecialchars($d['foto']); ?>" alt="Foto Lokasi" width="100" class="img-thumbnail" />
                <?php else: ?>
                    Tidak ada gambar
                <?php endif; ?>
            </td>
